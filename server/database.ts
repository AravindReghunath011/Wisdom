...
class PostgreSQLDatabase implements IDatabase {
  ...
  async getRandomQuestions(filters: {
    ...
    try {
      console.log(
        `🎲 Getting ${filters.count} random questions with filters:`,
        filters
      );
      ...
      const shuffledQuestions =
        this.shuffleQuestionsAndAnswers(selectedQuestions);

      console.log(
        `✅ Selected and shuffled ${shuffledQuestions.length} questions with randomized answers`
      );
   ...
   */
  private async optimizedRandomSelection(filters: {
    category?: string;
    difficulty?: string;
    count: number;
  }): Promise<Question[]> {
    try {
      ...
      console.log(
        "🔄 Using enhanced memory-based algorithm for optimal randomness"
      );
   ...
   */
  private async databaseLevelRandomSelection(filters: {
    category?: string;
    difficulty?: string;
    count: number;
  }): Promise<Question[]> {
    ...
    try {
      ...
      console.log(
        `📊 Database selection breakdown: ${primaryCount} primary, ${secondaryCount} secondary, ${tertiaryCount} tertiary`
      );

      // Step 1: Get primary candidates (filtered questions) using database random sampling
      if (filters.category && filters.category !== "All Categories") {
        const primaryQuery = this.buildEnhancedRandomQuery(
          filters.category,
        ...
        console.log(
          `✅ Selected ${primaryResults.length} questions from primary candidates`
        );

        // Step 2: Get secondary candidates (same difficulty, different categories)
        if (secondaryCount > 0) {
          const secondaryQuery = this.buildEnhancedRandomQuery(
            null,
            filters.difficulty || null,
            secondaryCount,
            filters.category
          );
          const secondaryResults = await this.executeRandomQuery(
            secondaryQuery
          );
          selectedQuestions.push(...secondaryResults);
          console.log(
            `✅ Selected ${secondaryResults.length} questions from secondary candidates`
          );
        }

        // Step 3: Get tertiary candidates (different difficulty, different categories)
        if (tertiaryCount > 0) {
          const tertiaryQuery = this.buildEnhancedRandomQuery(
            null,
            null,
            tertiaryCount,
            filters.category
          ...
          console.log(
            `✅ Selected ${tertiaryResults.length} questions from tertiary candidates`
          );
        }
        ...
        const randomQuery = this.buildEnhancedRandomQuery(
          null,
          null,
          filters.count
        );
        ...
        console.log(
          `✅ Selected ${randomResults.length} questions using pure random selection`
        );
      }
  ...
  async updateTeamBattle(
    ...
    await db.update(teamBattles).set(updates).where(eq(teamBattles.id, id));
