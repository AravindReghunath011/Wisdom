...
async function handleAuthenticate(clientId: string, event: GameEvent) {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `User ${userId} not found in storage, skipping online status update`
      );
    }
...
export async function broadcastTeamUpdates(gameSessionId: string) {
  try {
    ...
    console.log(
      ...
      teams.map((t) => ({ id: t.id, name: t.name, captainId: t.captainId }))
    );
    ...
    for (const client of connectedClients) {
      console.log(
        `Client ${client.id}: userId=${client.userId}, gameId=${client.gameId}, playerName=${client.playerName}`
      );
      // Send to authenticated clients (they might be on the team battle page)
      if (client.userId) {
        console.log(
          `Broadcasting to client ${client.id} (user ${client.userId})`
        );
    ...
    console.log(
      `Broadcasted team updates for session ${gameSessionId} to ${broadcastCount} connected clients`
    );
  } catch (error) {
  ...
  try {
    ...
    if (!displayName) {
      try {
        ...
        console.error("Error fetching user for team_option_selected:", error);
    ...
    console.error("Error handling team_option_selected:", error);
  }
}
      ...
      setTimeout(() => {
        try {
          ...
          if (captainClient) {
            ...
            console.warn(
              "No active captain client found to start team battle",
              battle.id
            );
          }
        } catch (err) {
          console.error("Error auto-starting team battle after countdown:", err);
        }
      }, countdownSeconds * 1000);
    }
  } catch (error) {
    console.error("Error handling team_battle_ready:", error);
  }
}
...
async function handleRecruitPlayer(clientId: string, event: GameEvent) {
  ...
  try {
    ...
    console.log("Processing recruitment:", {
      inviteeUserId,
      gameSessionId,
      recruiterId,
      recruiterName,
    ...
    console.log(
      "User teams in any session:",
      userTeamsInAnySession.length,
      userTeamsInAnySession.map((t) => ({
        id: t.id,
        name: t.name,
    ...
    const inviteeTeamMembership = existingTeams.find((team) => {
      ...
      if (isMember) {
        console.log(
          `User ${inviteeUserId} found in team ${team.id} in current session`
        );
      }
      return isMember;
    });
    ...
    if (inviteeTeamMembership) {
      console.log(
        `Recruitment BLOCKED: User ${inviteeUserId} is already in team ${inviteeTeamMembership.id} in current session`
      );
      sendToClient(clientId, {
        type: "error",
    ...
    console.log(
      `Recruitment check PASSED: User ${inviteeUserId} is not in any active team`
    );
    ...
    if (!recruiterTeam && userTeamsInAnySession.length > 0) {
      ...
      console.log(
        "Found recruiter team in different session, updating gameSessionId to:",
        recruiterTeam.gameSessionId
      );
      ...
      if (inviteeInUpdatedTeams) {
        console.log(
          `Recruitment blocked after session update: User ${inviteeUserId} is in team ${inviteeInUpdatedTeams.id}`
        );
        sendToClient(clientId, {
          type: "error",
    ...
    console.log(
      ...
      recruiterTeam
        ? {
            id: recruiterTeam.id,
            name: recruiterTeam.name,
            members: recruiterTeam.members.length,
          }
        : "None"
    );
    console.log(
      "Recruitment logic check - recruiterTeam exists:",
      !!recruiterTeam,
      "existingTeams.length:",
      existingTeams.length
    );
    ...
    if (existingTeams.length === 2) {
      ...
      console.log(
        "Team with members creating opposing team captain invitation"
      );
...
async function processTeamBattleAnswers(gameId: string) {
  ...
  if (!currentQuestion || !Array.isArray((currentQuestion as any).answers)) {
    console.error(
      "processTeamBattleAnswers: missing currentQuestion or answers for gameId",
      gameId,
      {
        currentIndex,
        questionsLength: gameSession.questions.length,
      }
    );
    return;
  }
...
async function handleGetGameState(clientId: string, event: GameEvent) {
  ...
  try {
    console.log(event.gameSessionId,'gamesessionid')
