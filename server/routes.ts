...
import { setupWebSocketServer, sendToUser, getOnlineUserIds } from "./socket";
import { z } from "zod";
  ...
  app.patch("/api/teams/:id", ensureAuthenticated, async (req, res) => {
    try {
      ...
      if (team.captainId !== req.user?.id) {
        return res
          .status(403)
          .json({ message: "Only team captain can update team" });
      }
      ...
      try {
        ...
        if (status === "accepted") {
          if (updatedInvitation.invitationType === "opponent") {
            ...
            // Find the inviter's existing team battle (Team A only)
            const inviterBattles = await database.getTeamBattlesByUser(
              invitation.inviterId,
              "forming"
            );

            // Find battle that doesn't have Team B yet
            const existingBattle = inviterBattles.find(
              (b) => !b.teamBCaptainId || !b.teamBName
            );
