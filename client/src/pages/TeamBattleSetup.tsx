...
const TeamBattleSetup: React.FC = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (storedSessionId) {
        ...
        console.log("Restored existing game session ID:", storedSessionId);
        return;
      }
    }
  ...
  useEffect(() => {
    ...
    if (userTeam) {
      const userMember = userTeam.members.find(
      ...
      console.log("User team details:", {
        ...
        members: userTeam.members.map((m: TeamMember) => ({
          userId: m.userId,
          username: m.username,
          role: m.role,
        })),
  ...
  useEffect(() => {
    if (user?.id && refetchTeams && refetchInvitations) {
      console.log("Setting up WebSocket for team battle setup");
      const socket = setupGameSocket(user.id);

      const handleMessage = (event: MessageEvent) => {
        try {
          const data = JSON.parse(event.data);
          console.log("WebSocket message received in TeamBattleSetup:", data);

          switch (data.type) {
            case "opponent_accepted_invitation":
              console.log("Opponent accepted invitation via WebSocket:", data);
              toast({
                title: "Opponent Accepted!",
                description:
                  data.message || "Your opponent has accepted the invitation!",
              });
              // Refetch teams and invitations
              refetchTeams();
              refetchInvitations();
              break;

            case "teams_updated":
              console.log("Teams updated via WebSocket:", data);
              if (data.message) {
                toast({
                  title: "Teams Updated",
              ...
              // Refetch teams when teams are updated
              refetchTeams();
              refetchInvitations();
              break;
          }
      ...
      return () => {
        console.log("Cleaning up WebSocket listener for team battle setup");
        socket.removeEventListener("message", handleMessage);
      };
    }
  }, [user?.id, refetchTeams, refetchInvitations, toast]);
  ...
  const respondToInvitationMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      if (variables.status === "accepted") {
        // Check if this is an opponent invitation (has teamBattle)
        const isOpponentInvitation = !!data.teamBattle;

        if (isOpponentInvitation) {
          console.log("Opponent invitation accepted - team battle created");
          toast({
          ...
          console.log("Teammate invitation accepted");
          toast({
            title: "Invitation Accepted! 2",
  ...
  const leaveTeamMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      const newSessionId = generateNewSessionId();
      console.log("Generated new session after leaving team:", newSessionId);
      refetchTeams();
      refetchInvitations();
    },
    onError: (error: any) => {
  ...
  const handleSendInvitation = (
    ...
    if (!userTeam) {
      toast({
        title: "Error",
        description: "You must be in a team to send invitations 2",
        variant: "destructive",
      });
      return;
    }
    ...
    console.log(
      "Sending invitation with teamId:",
      userTeam.id,
      "inviteeId:",
      userId,
      "isCaptainInvitation:",
  ...
  const handleAcceptCaptainInvitation = () => {
    if (pendingInvitationId && newTeamName.trim()) {
      ...
      console.log(
        "Accepting captain invitation with team name:",
        newTeamName.trim()
      );
      ...
      console.log(
        "Force refetching teams immediately after captain invitation acceptance"
      );
      refetchTeams();
      refetchInvitations();
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "Switching to existing team battle session:",
        userTeam.gameSessionId
      );
