...
const TeamBattleSetup: React.FC<TeamBattleSetupProps> = ({
      ...
      try {
        ...
        switch (data.type) {
          case "opponent_accepted_invitation": {
            console.log(
              "Opponent accepted invitation via WebSocket:",
              data
            );
          ...
          case "team_ready_status": {
            if (data.teamAReady !== undefined && data.teamBReady !== undefined) {
              setReadyStatus({
  ...
  }, [countdown, hasNavigatedToGame, gameSessionId, userTeam, onClose, setLocation]);
  ...
  const handleReadyToPlay = async () => {
    ...
    try {
      ...
      console.error("Error sending team ready event:", error);
      toast({
        title: "Error",
  ...
  const [currentStage, setCurrentStage] = useState<"create-team" | "invite-opponent" | "invite-teammates">(
    !userTeam ? "create-team" : !opponentAccepted ? "invite-opponent" : "invite-teammates"
);
  useEffect(() => {
  if (!userTeam) {
  ...
  const sendInvitationMutation = useMutation({
    mutationFn: async (data: {
      ...
      if (!gameSessionId) {
        throw new Error("No active game session. Please create or join a team first.");
      }
    ...
    onSuccess: async (data, variables) => {
      if (variables.status === "accepted") {
        ...
        try {
          ...
          console.error("Failed to fetch updated teams after acceptance", error);
        }
        ...
        if (
          ...
          const hasBothTeams = Array.isArray(latestTeams)
            ...
            : teams.length >= 2;
          setCurrentStage(hasBothTeams ? "invite-teammates" : "invite-opponent");
        }

        toast({
          title: "Invitation Accepted!",
          description: "You have joined the team battle.",
        });
      } else {
        toast({
  ...
  const handleInviteOpponent = (userId: number) => {
    if (!userTeam) {
      toast({
        title: "Error",
    ...
    console.log("handleinviteopponent", userId);
  ...
  const handleInviteTeammate = (userId: number) => {
    if (!userTeam) {
      toast({
        title: "Error",
                    ...
                    {availableOpponents.map((player) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Button
                            size="sm"
                            onClick={() => handleInviteOpponent(player.id)}
                            disabled={
                    ...
                    {availableTeammates.map((player) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Button
                            size="sm"
                            onClick={() => handleInviteTeammate(player.id)}
                            disabled={
                              sendInvitationMutation.isPending ||
                              !!pendingInvitation ||
                          ...
                          >
                            {pendingInvitation ? "Invited" : "Invite to Team"}
                          </Button>
                        </div>
                      );
                    })}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-blue-900">
                          Invitation from {invitation.inviterUsername || "User"}
                        </p>
                        <p className="text-sm text-blue-700">
                          {invitation.inviterUsername || "Someone"} has invited
                          you to join a team battle
                        </p>
                      </div>
